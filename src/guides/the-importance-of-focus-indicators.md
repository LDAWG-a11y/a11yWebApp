...
We only covered a small number of UI elements above, as there are of course far too many to go over in a single article. The aim of this article was not necessarily to state you should use one type of indicator over another, more to discuss different types of indicators and discuss the pros and cons of each. 

Ultimately there are folks that do not use a mouse, trackpad or touchscreen and they need to see where their focus is at all times, it's as important to them as the cursor is to mouse users, so we should never take it away from them. Some of those users may have low-vision, in which case they may only be able to use our sites if there is a highly-perceptible focus indicator, so we need to ensure that when an item receives focus, it is clear it is the focused item and also, don't obscure the label or icon of the element that has focus, as it's probably super useful to actually see what has focus.
