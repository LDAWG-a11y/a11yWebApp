...
* They both have a value that points to an ID of another element in the DOM
...
I do tend to use `aria-controls` though and I use that in basic disclosure patterns, so a button that when interacted with will reveal some previously hidden content. We'll start with this property, as more often than not, this may be the one you actually need, but read on regardless.
...
This property is of course not restricted to just mobile navigations, it is also used on accordions, typeahead type searched, custom select inputs and much more. [The ARIA 1.2 spec provides the details about the property](https://www.w3.org/TR/wai-aria-1.2/#aria-controls).
...
It will look exactly the same in the DOM, as ARIA isn't really for the DOM, it doesn't manipulate it or add functionality etc, so maybe this is less evident as we could be looking in the wrong place? let's take our earlier example of a mobile nav and swap the `aria-controls` property for the `aria-owns` property, this is availble in the following code example:
...
In just code, that perhaps looks quite harmless to many and could perhaps be missed quite easily. Let's see how it looks in the DOM, I'm going to duplicate it and add aria-controls back in on the first, I'll change text labels and IDs of course, but I want to this for my comparison:
...
Well the first thing that is happening is essentially because I did not actually make this interactive, so the list itself is actually exposed to the accessibility tree, as it is not hidden, then its contents get added to the accessible name of the element that owns it.

If we were to just hide that element with `display: none`; in CSS, it would not be read out, but then what if a user leaves it open after expanding it and focuses back on the button, later? Then it will be read out again and depending on what screen reader and browser combo is used, the results will likely range from no different to meh.

So, Safari and VoiceOver on MacOS do not support this property at all, so if you are testing with VoiceOver on Mac, you won't hear anything out of the ordinary in Safari.

But, what if we stay on the Mac and use Firefox or even the commit an accessibility faux pas and try it with Chrome? In both instances the property is communicated, the name is read out on Chrome the same as the accessibility pane indicated it would: "I own something elsewhere", which is obviously not great at all and would be even worse if there were lots of links in that list.
...
Note: I don't usually test VoiceOver in Chrome, but I just wanted to show how it is exposed and it isn't in Safari.

#### Ok that's bad, why does it actually do that?

Essentially, by using `aria-owns`, what we see in the DOM is not the structure the accessibility tree creates, you're essentially telling the accessibility tree to interpret your button as this:
...
The output of the above VoiceOver text viewer sounds pretty bad, I'm sure you will agree, again this is in Firefox and the output is as follows: "I own something, bullet, elsewhere, bullet, elsewhere 2, bullet, elsewhere 3, collapsed, button, group". The more things that get added to that list get added to the accessible name and whilst that list won't always be exposed to the accessibility tree (assuming it hides correctly) the thing is, it's not always hidden, so the moment you interact with that button and the list is exposed to the accessibility tree, everything is read out, the trigger and all of the contents of that list, which I doubt any screen reader user would thank you for.

I concede to not being thorough here, I haven't tested this on NVDA or JAWS and that's not due to me being lazy, I'm not testing support as such, I'm, just explaining how it works. [We have accordions on this site, you can find one here on the guides page](https://www.makethingsaccessible.com/guides/), if you open the DevTools in anything other than Safari and change the aria-controls property on the button to aria-owns, then pop on your screen reader and expand the accordion, try that across a couple of browsers and see if you get the mess that I got in Firefox and VoiceOver.
...
We can actually easily find out what aria-owns does to the element in the accessibility tree, in the screenshot, the button with the `aria-controls` property has the list as a sibling, which is what we want for this type of relationship, but the button with `aria-owns` has the list nested, which is definitely not what we want.

#### Are there any use cases for ARIA Owns?

Of course, there are many. the folks that work on ARIA are smart, they work hard to get this stuff into the wild and that will all be based upon gaps in HTML for assistive tech, much of this information would be based upon what users of assistive technologies have reported to them. So, they don't just create attributes for the sake of creating them, so there are likely lots of valid use cases for this property.

I'm somewhat struggling to think of one, but I guess (take this with a pinch of salt), if you had a drawer-type navigation, which wasn't adjacent to the trigger element and you needed it to be exposed as in the same `<nav>` landmark as the trigger, maybe that would be a use case for it? I guess that would look something like this (please don't copy this code):
...
Essentially what I did here was referenced the ID of the drawer, which for some bizarre reason is situated at the bottom of the DOM, [the ARIA 1.2 spec for ARIA owns](https://www.w3.org/TR/wai-aria-1.2/#aria-owns) states that "If an element has both `aria-owns` and DOM children then the order of the child elements with respect to the parent/child relationship is the DOM children first, then the elements referenced in `aria-owns`", which means we don't need to build up the list in the same we would for `aria-labelledby`'s accessible name. So the trigger would come first in the reading order and then the actual drawer, as this behaviour is the expected behaviour.
