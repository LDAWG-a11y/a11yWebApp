...
So we need a function to handle setting the active tab and displaying its corresponding panel, whilst setting all other tabs to be non-active and their corresponding panels to `hidden`, we will do this in our `setActiveTab()` function, which will accept the current `tab` a user has clicked or selected
