...
Firstly establish if there is any instructional text, this could be a sentence at the top of the form that says "All fields are required" or it could be the significantly better "(required)" appended to the label, or indeed, anything in between. If there is an absence of instruction and it is ambiguous (a regular form) which (if any) fields are required and which (if any) are optional the we should explore further.

My first technique is to attempt to submit a "clean" form (if the submit button is present and not disabled", this will often, but not always trigger some form of validation errors, be they native HTML error messages or custom messages. This will typically inform me which fields are required.

I then search further by looking at the DOM and searching for the existence of `required` or `aria-required`, although, never rely just on this, one of these should be used, but in reality any attribute could be used for validation.

I will then add some made up input to only the fields that were identified as required after my empty submission and hope for a successful submission.

If any other fields require a particular format and that is not explained in text, then I will also take note of those.

If it is one of those forms that does not allow you to submit until the super secret validation enigma has been solved, and there are no required or aria-required attributes on the fields, then iI just write it up as a whole and state I do not know which fields are required. I ask for clarity on which fields are required and which are optional and I state that at a minimum, I need to have at least a solitary form level instruction to satisfy this. Remember, this SC is only concerned with the presence of that instruction, it does not care if it is programmatically associated or otherwise accessible, that is mostly a 1.3.1 issue,

##### Solution

In an ideal world "(required)" would be added to each required field on a form where some are required and others optional. Adding "(optional)" to optional fields in addition to "(required)" is sometimes recommended, I'm not persoanlly sure how much better that makes things for users, i do not know if it is better to just specify which are optional, which are required or both, I feel like just saying "(required)" on only the required fields is explicit enough, in that if it isn't present on a field, then it's clearly not required.

In my opinion, a better strategy is just not to have optional fields, if the organisation do not legitimately need that data, then why ask for it at all?
...
Axe finds these issues with ease, as will other semi-automated tools. These issues could be picked up via code inspection, the Element Selector, the DevTools accessibility tree or indeed a screen reader. In the DevTools there are also red squiqqly underlines under labels that lack an associated input, however, this isn't entirely accurate, as one does have an accessible name, I just did it completely back to front. Also, when we use the toggle to fix the site, those red squiggles remain so are no longer providing any useful information.
