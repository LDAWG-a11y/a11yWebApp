...
Where digital accessibility is concerned, semi-automated testing is using third-party tools to check for any detectable accessibility issues on webpages (usually 1 page at a time, hence why it is semi-automated). These tools provide a vital first line of defence against many common accessibility issues, such as:
...
The list above is by no means exhaustive, but they are some of the most common issues detected. It’s also worth noting that these tools are not perfect, no tool can completely test a page, in fact, automated or semi-automated tools typically only pick up around 40% of known accessibility issues, but they are a vital first line of defence and it’s great practice to familiarise yourself with them and check your pages or sites to get a basic overview of the accessibility.

## Tell me about the tools

Every accessibility professional will have a toolkit, usually a folder full of bookmarks, several browser extensions and some bookmarklets that test specific things, these will naturally differ between individuals, so this list is again not exhaustive, but these are the tools we typically use:

There's a couple of considerations to be aware of before we delve into the tools:

* Some of these tools reside in the "Dev Tools", when we reference "Dev Tools", we're referring to the browsers' code inspector, where we can view all of the HTML, CSS and much more for the current page. To access the Dev Tools, you can right-click anywhere on the page and selecting “Inspect” or “Inspect Element”, alternatively, you can just press the F12 key and it will open up (depending on your keyboard, you may need to press the function or fn key, along with F12).
...
Further down in the panel, there are some options which will help you to resolve the issue, in this case it’s as simple as adding some text, that will inform users of the purpose of that link, i would likely suggest alt text here, as these are functional images.
...
There is A “Summary” section, which displays a tabbed interface, each tab, with the exception of the “Summary” tab provides information about the page as a whole:
...
The “Structure” tab displays the structural content of the page, things like heading hierarchy and landmarks. This enables us to check, at a glance that:
...
* Additionally, it is also possible to check the heading hierarchy, it’s important to some users that headings are marked up using the correct <h\[1-6]> tags and that they’re in a logical order. A page should always have a <h1>, major sections would have <h2>s, subsections <h3>s and so forth.
...
8. Once Edge restarts, much like Axe, ARC lives in the DevTools, which can be accessed by right clicking anywhere on the page or pressing F12
...
* Check Text Spacing (Necessary to meet WCAG 1.4.12 Text Spacing), all text on the page should visibly increase, Line Height, Letter Spacing, Word Spacing and Paragraph Spacing
...
No installation is necessary for Lighthouse, it’s built right into Chromium-based browsers, Chrome, Edge, Brave etc. It can be accessed within the DevTools panel, by right-clicking anywhere on the page and selecting “Inspect” or by pressing F12.

Lighthouse will be one of the tabs at the top of the DevTools panel, clicking that will display the Lighthouse interface.



There are a number of options we can test against, including Best practices, SEO and Performance, progressive Web Apps & Accessibility, I’m just going to test against accessibility in this guide, but if I were developing a site, I would definitely test against Performance, SEO and Best Practices, as performance definitely does link to accessibility, as people with disabilities are a marginalised group and more likely to experience socio-economic related poverty, which means they cannot afford the latest and greatest tech, so a page that takes 15 seconds to become interactive on an iPhone 14 Pro Max on 5G due to Excessive file sizes and lack of image optimisations etc, may be completely unusable for a user accessing on an old low-powered smart phone, using only 3G.
...
After the test has run, a score will be given against each category, as this guide is only focusing on the Accessibility category, I have a score of 88 out of 100. Which to the uninitiated may seem fairly good, as a point of interest, Lighthouse actually uses Axe’s engine (Axe Core) to perform the tests, but it doesn’t use the latest version of Axe Core and in any instance, I can see it has missed quite a few issues that the other tools detected, as I have been testing the same page each time.
...
**Aside:** I tend to find that having Axe installed conflicts with page performance scores, so if you're going for the "Four hundreds" (100 score for all but Progressive Web App), it may be useful to test in Incognito Mode, in another persona or disable other extensions).
...
Clicking the Accessibility Insights button will give us 3 main options:
...
Or, [if you access the Validator](https://validator.w3.org/) itself, there are options to “Validate by URI”, “Validate by File upload” or “Validate by Direct Input” (paste the page source into the validator).
...
* ARC Toolkit to check Text Spacing, page Reflow and send the DOM to the validator
* The HTML validator
